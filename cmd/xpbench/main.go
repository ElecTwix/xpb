...
func printTable(results []Result) {
	...
	for _, cat := range categories {
		...
		for f := range formats { sortedFormats = append(sortedFormats, f) }
		sort.Strings(sortedFormats)
		
		// Prioritize XPB
		sort.Slice(sortedFormats, func(i, j int) bool {
			if strings.Contains(sortedFormats[i], "XPB") && !strings.Contains(sortedFormats[j], "XPB") { return true }
			if !strings.Contains(sortedFormats[i], "XPB") && strings.Contains(sortedFormats[j], "XPB") { return false }
		...
		for _, fmtName := range sortedFormats {
			for _, op := range []string{"Encode", "Decode"} {
				...
				sizeStr := "-"
				if size > 0 { sizeStr = fmt.Sprintf("%d B", size) }
...
func simplifyFormatName(name string) string {
	if strings.Contains(name, "XPB") { return "XPB V2" } // Group JIT/Manual together? 
	// Actually, we want to see JIT vs Manual.
	// But keeping it simple for the consolidated table:
	if strings.Contains(name, "JIT") { return "XPB V2 (JIT)" }
	if strings.Contains(name, "Manual") { return "XPB V2 (Man)" }
	return name
}
