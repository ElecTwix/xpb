...
// DefaultRuntimeImport is the module specifier the generated TypeScript imports
// the xpb runtime from when no override is supplied.
const DefaultRuntimeImport = "@xpb/runtime"

// Options controls TypeScript generation.
type Options struct {
	// RuntimeImport overrides the module specifier in the generated runtime
	// import (`import { ... } from '<RuntimeImport>'`). Empty means
	// DefaultRuntimeImport. Use this to point generated code at a vendored or
	// relative runtime path without post-processing the output.
...
// Generate generates TypeScript code for the given file AST using default
// options.
func Generate(file *ast.File) ([]byte, error) {
	return GenerateWithOptions(file, Options{})
}

// GenerateWithOptions generates TypeScript code for the given file AST using the
// supplied options. Unset fields fall back to their documented defaults.
func GenerateWithOptions(file *ast.File, opts Options) ([]byte, error) {
	...
	g := &Generator{enums: ast.NewEnumSet(file), opts: opts}
...
func (g *Generator) generate(file *ast.File) ([]byte, error) {
	...
	g.printf("import { Encoder, Decoder, MaxDecodeDepth, MaxDecodeDepthExceededError } from '%s';\n\n", g.opts.RuntimeImport)
