...
// These tests document and ENFORCE the safety boundary of --go-zero-copy-bytes:
//
//   - val style (generated with --go-zero-copy-bytes) decodes bytes via
...
//     lives.
//   - ptr style (default, no zero-copy) decodes bytes via ReadBytes, which
//     COPIES. The decoded slice owns its memory; mutating the input is NOT
//     visible.
//
...
// TestAliasing_ZeroCopyValAliasesInput proves the zero-copy (val) decode aliases
// the input buffer: mutating the source buffer changes the decoded Payload.
func TestAliasing_ZeroCopyValAliasesInput(t *testing.T) {
	...
	// Because val decode is zero-copy, the mutation must be visible through the
	// decoded slice. If this fails, --go-zero-copy-bytes stopped aliasing.
	if m.Payload[0] != 0x00 {
		t.Fatalf("zero-copy decode did NOT alias input: Payload[0]=%#x, want 0x00 (mutation must be visible)", m.Payload[0])
	}
}

// TestAliasing_DefaultPtrDoesNotAliasInput proves the default (ptr) decode does
// NOT alias: it copies the payload, so mutating the source buffer leaves the
// decoded slice untouched.
func TestAliasing_DefaultPtrDoesNotAliasInput(t *testing.T) {
	...
	// Default decode copies; the mutation must NOT be visible.
	if (*m.Payload)[0] != payloadMarker[0] {
		t.Fatalf("default decode aliased input: Payload[0]=%#x, want %#x (must be an independent copy)", (*m.Payload)[0], payloadMarker[0])
	}
}
